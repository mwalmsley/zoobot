...
# debug mode
# sbatch --job-name=dr5_py_debug_$SEED --export=GPUS=1,DEBUG_STRING='--debug' $TRAIN_JOB

# minimal hardware - 1 gpu, no mixed precision
# (not specifying a string will default to not doing it)
# sbatch --job-name=dr5_py_min_b512_lrdef_repsampfalse_$SEED --export=MIXED_PRECISION_STRING=--mixed-precision,GPUS=1,SEED=$SEED $TRAIN_JOB

# otherwise full hardware (standard setup) - 2 gpus, mixed precision
sbatch --job-name=dr5_py_gr_b512_temp_test_noseed_2pgu_$SEED --export=MIXED_PRECISION_STRING=--mixed-precision,GPUS=2,SEED=$SEED $TRAIN_JOB
