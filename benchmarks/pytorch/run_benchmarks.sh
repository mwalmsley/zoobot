...
# debug mode
# sbatch --job-name=dr5_py_debug_$SEED --export=ARCHITECTURE=efficientnet,BATCH_SIZE=256,DATASET=gz_decals_dr5,GPUS=1,SEED=$SEED,DEBUG_STRING='--debug' $TRAIN_JOB

# minimal hardware - 1 gpu, no mixed precision
# (not specifying a string will default to not doing it)
# sbatch --job-name=dr5_py_min_$SEED --export=ARCHITECTURE=efficientnet,BATCH_SIZE=256,DATASET=gz_decals_dr5,GPUS=1,SEED=$SEED $TRAIN_JOB

# otherwise full hardware (standard setup) - 2 gpus, mixed precision
# sbatch --job-name=dr5_py_gr_$SEED --export=ARCHITECTURE=efficientnet,BATCH_SIZE=256,DATASET=gz_decals_dr5,MIXED_PRECISION_STRING=--mixed-precision,GPUS=2,SEED=$SEED $TRAIN_JOB
